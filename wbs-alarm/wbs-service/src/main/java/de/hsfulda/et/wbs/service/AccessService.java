...
public class AccessService {
    ...
    public <T> T hasAccessOnZielort(final WbsUser user, final Long zielortId, final Supplier<T> supplier) {

        Long counts = repo.findTraegerByUserAndZielortId(user.getId(), zielortId);
        return evaluteCount(counts, zielortId, "Zielort", supplier);
    }

    public <T> T hasAccessOnTraeger(final WbsUser user, final Long traegerId, final Supplier<T> supplier) {

        Long counts = repo.findTraegerByUserAndTraegerId(user.getId(), traegerId);
        return evaluteCount(counts, traegerId, "Träger", supplier);
    }

    public <T> T hasAccessOnKategorie(final WbsUser user, final Long kategorieId, final Supplier<T> supplier) {
    ...
    public <T> T hasAccessOnGroesse(final WbsUser user, final Long groesseId, final Supplier<T> supplier) {
    ...
    public <T> T hasAccessOnTransaktion(WbsUser user, TransaktionDto dto, final Supplier<T> supplier) {
        ...
        dto.getPositions()
                .forEach(p -> {
                    checkCount(repo.findTraegerByUserAndGroesseId(user.getId(), p.getGroesse()), p.getGroesse(),
                            "Größe");
                });
